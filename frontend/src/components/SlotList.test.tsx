...
describe('SlotList', () => {
  it('shows loader when loading', () => {
    renderWithProviders(
      <SlotList slots={[]} loading={true} selectedSlot={null} onSelect={vi.fn()} />,
    );
  ...
  it('shows empty message when no slots', () => {
    renderWithProviders(
      <SlotList slots={[]} loading={false} selectedSlot={null} onSelect={vi.fn()} />,
    );
  ...
  it('shows available slot as clickable button and calls onSelect', async () => {
    ...
    renderWithProviders(
      <SlotList slots={[availableSlot]} loading={false} selectedSlot={null} onSelect={onSelect} />,
    );
    expect(screen.getByText(/Свободен/i)).toBeInTheDocument();
    // Click the only button (the available slot); time is formatted in local TZ so we avoid asserting exact time
  ...
  it('shows booked slot as "Занято" without button', () => {
    renderWithProviders(
      <SlotList slots={[bookedSlot]} loading={false} selectedSlot={null} onSelect={vi.fn()} />,
    );
